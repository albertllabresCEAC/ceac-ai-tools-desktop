 ...
 *   c0 – checkbox
 *   c1 – <th> name link (contains span.userinitials + text node with full name)
 *   c2 – email (plain text)
 *   c3 – roles (via a.quickeditlink own text)
 *   c4 – groups (via a.quickeditlink own text)
 *   c5 – last access
 *   c6 – enrolment status
 */
@Component
public class MoodleCourseParticipantsParser {
    ...
    private CourseParticipant parseRow(Element row) {
        ...
        // Roles: c3 — text of a.quickeditlink (ownText avoids the pencil icon label)
        String roles = quickEditText(row, "td.c3");

        // Groups: c4 — same pattern
