package tools.ceac.ai.mcp.campus.interfaces.mcp;
...
import tools.ceac.ai.mcp.campus.application.service.CreateQuizMultichoiceQuestionUseCase;
import tools.ceac.ai.mcp.campus.application.service.DeleteQuizSlotUseCase;
import tools.ceac.ai.mcp.campus.application.service.DeleteQuizUserOverrideUseCase;
...
import tools.ceac.ai.mcp.campus.application.service.GetConversationMessagesUseCase;
import tools.ceac.ai.mcp.campus.application.service.GetConversationsUseCase;
...
import tools.ceac.ai.mcp.campus.application.service.GetDashboardUseCase;
import tools.ceac.ai.mcp.campus.application.service.GetGradeUseCase;
import tools.ceac.ai.mcp.campus.application.service.GetMessageRecipientsUseCase;
import tools.ceac.ai.mcp.campus.application.service.GetQuizAttemptReviewUseCase;
import tools.ceac.ai.mcp.campus.application.service.GetQuizCommentUseCase;
import tools.ceac.ai.mcp.campus.application.service.GetQuizConfigUseCase;
import tools.ceac.ai.mcp.campus.application.service.GetQuizQuestionCategoriesUseCase;
import tools.ceac.ai.mcp.campus.application.service.GetQuizQuestionUseCase;
import tools.ceac.ai.mcp.campus.application.service.GetQuizResultsUseCase;
import tools.ceac.ai.mcp.campus.application.service.GetQuizStructureUseCase;
import tools.ceac.ai.mcp.campus.application.service.GetQuizUserOverridesUseCase;
import tools.ceac.ai.mcp.campus.application.service.GetUserProfileUseCase;
import tools.ceac.ai.mcp.campus.application.service.MarkConversationAsReadUseCase;
import tools.ceac.ai.mcp.campus.application.service.ReplyToMessageUseCase;
import tools.ceac.ai.mcp.campus.application.service.SaveQuizCommentUseCase;
import tools.ceac.ai.mcp.campus.application.service.SaveQuizUserOverrideUseCase;
import tools.ceac.ai.mcp.campus.application.service.SendNewMessageUseCase;
...
import tools.ceac.ai.mcp.campus.domain.model.ConversationDetail;
import tools.ceac.ai.mcp.campus.domain.model.CourseDetail;
import tools.ceac.ai.mcp.campus.domain.model.DashboardSnapshot;
import tools.ceac.ai.mcp.campus.interfaces.api.dto.DeleteQuizSlotResponse;
...
public class CampusMcpTools {
    ...
    @Tool(description = "Estado del servidor y si hay sesión activa en el campus")
    ...
    @Tool(description = "Perfil del usuario autenticado: nombre, userId, email, idioma y mensajes no leídos")
    ...
    @Tool(description = "Lista de participantes matriculados en un curso con nombre, email, rol y último acceso. Requiere el courseId.")
    ...
    @Tool(description = "Perfil completo de un usuario (alumno) por su userId: nombre, email, país, cursos matriculados y accesos.")
    ...
    @Tool(description = "Lista de entregas de alumnos para una tarea. Requiere el assignmentId (id del módulo Moodle).")
    ...
    @Tool(description = "Calificación y feedback actual de la entrega de un alumno. Requiere assignmentId y userId.")
    ...
    @Tool(description = """
            ...
            ANTES DE EJECUTAR: muestra al usuario un resumen (alumno, nota, feedback) y pide confirmación mediante un selector Sí/No. Solo procede si elige Sí.""")
    ...
    @Tool(description = "Resultados de todos los intentos de un cuestionario. Requiere el quizId (id del módulo Moodle).")
    public Object getQuizResults(String quizId) {
        return getQuizResultsUseCase.execute(quizId);
    }

    @Tool(description = "Revisión detallada de un intento de cuestionario: preguntas, respuestas, puntuaciones. Requiere attemptId.")
    public Object getQuizAttempt(String attemptId) {
        return getQuizAttemptReviewUseCase.execute(attemptId);
    }

    // ── CUESTIONARIOS — CORRECCIÓN ───────────────────────────────────────────

    @Tool(description = "Datos de corrección de una pregunta de un intento (enunciado, respuesta correcta, puntuación actual, comentario). Requiere attemptId y slot.")
    public Object getQuizComment(String attemptId, String slot) {
        return getQuizCommentUseCase.execute(attemptId, slot);
    }

    @Tool(description = """
            Guarda la puntuación y comentario de una pregunta de un intento. Requiere attemptId, slot, mark (número como string) y comentario.
            ANTES DE EJECUTAR: muestra al usuario un resumen (intento, slot, nota, comentario) y pide confirmación mediante un selector Sí/No. Solo procede si elige Sí.""")
    ...
    // ── CUESTIONARIOS — OVERRIDES ────────────────────────────────────────────

    @Tool(description = "Lista de excepciones de usuario configuradas en un cuestionario. Requiere el cmid del módulo.")
    public Object getQuizOverrides(String cmid) {
        return getQuizUserOverridesUseCase.execute(cmid);
    }

    @Tool(description = """
            Crea o actualiza una excepción de usuario en un cuestionario.
            ...
            ANTES DE EJECUTAR: muestra al usuario un resumen de los cambios y pide confirmación mediante un selector Sí/No. Solo procede si elige Sí.""")
    ...
    @Tool(description = """
            Elimina una excepción de usuario de un cuestionario. Requiere el overrideId.
            ANTES DE EJECUTAR: muestra al usuario los datos de la excepción y pide confirmación mediante un selector Sí/No. Solo procede si elige Sí.""")
    ...
    @Tool(description = "Mensajes de una conversación con sus miembros. Requiere convId y currentUserId.")
    ...
    @Tool(description = """
            Marca una conversación como leída. Requiere convId y userId.
            ANTES DE EJECUTAR: pide confirmación mediante un selector Sí/No. Solo procede si elige Sí.""")
    ...
    @Tool(description = """
            ...
            ANTES DE EJECUTAR: muestra al usuario el destinatario, asunto y contenido del mensaje y pide confirmación mediante un selector Sí/No. Solo procede si elige Sí.""")
    ...
    @Tool(description = """
            Envía un nuevo mensaje a uno o varios destinatarios. Requiere lista de recipientIds (solo el número, sin 'user-'), subject y content (HTML).
            ANTES DE EJECUTAR: muestra al usuario la lista de destinatarios, asunto y contenido del mensaje y pide confirmación mediante un selector Sí/No. Solo procede si elige Sí.""")
    ...
    // ── CUESTIONARIOS — ESTRUCTURA Y EDICIÓN ────────────────────────────────

    @Tool(description = "Configuración de un cuestionario (fechas, límite de tiempo, intentos, calificación, contraseña, etc.). Requiere el cmid del módulo.")
    public Object getQuizConfig(String cmid) {
        return getQuizConfigUseCase.execute(cmid);
    }

    @Tool(description = "Estructura de un cuestionario: slots (preguntas) con su número, página, tipo, nombre y puntuación máxima. Requiere el cmid del módulo.")
    public Object getQuizSlots(String cmid) {
        return getQuizStructureUseCase.execute(cmid);
    }

    @Tool(description = """
            Lista de categorías disponibles del banco de preguntas para un cuestionario.
            Requiere el cmid del módulo.
            El campo 'categoryValue' de cada categoría es el que hay que pasar al crear una pregunta.""")
    public Object getQuizQuestionCategories(String cmid) {
        return getQuizQuestionCategoriesUseCase.execute(cmid);
    }

    @Tool(description = "Datos completos de una pregunta (enunciado, respuestas, feedback, configuración). Requiere questionId y el cmid del cuestionario.")
    public Object getQuestion(String questionId, String cmid) {
        return getQuizQuestionUseCase.execute(questionId, cmid);
    }

    @Tool(description = """
            Crea una nueva pregunta de tipo opción múltiple en el banco de preguntas de un cuestionario.
            ...
            Fracciones: decimales sobre 1 con punto como separador. Ejemplos: 1.0 = correcta, 0.0 = incorrecta sin penalización, -0.5 = penalización 50%. NUNCA usar porcentajes enteros ni coma como separador decimal.
            ANTES DE EJECUTAR:
            1. Llama a getQuizQuestionCategories(cmid) y presenta al usuario la lista de categorías disponibles (campo name) para que seleccione una mediante un selector. Usa el campo categoryValue de la categoría seleccionada.
            2. Muestra al usuario un resumen de la pregunta y pide confirmación mediante un selector Sí/No. Solo procede si elige Sí.""")
    ...
    @Tool(description = """
            ⚠️ Esta herramienta SOLO crea la pregunta en el banco, NO la añade al cuestionario ni la hace visible a los alumnos; para crear una pregunta y añadirla directamente al test usa createQuizMultichoiceQuestion.
            Crea una nueva pregunta de tipo opción múltiple directamente en el banco de preguntas (sin añadirla a ningún slot del cuestionario).
            Requiere: cmid y los datos de la pregunta.
            Campos obligatorios en question: name, questionText, defaultMark, single ('1'=una respuesta, '0'=varias), answers (lista con text, fraction y feedback).
            Fracciones: decimales sobre 1 con punto como separador. Ejemplos: 1.0 = correcta, 0.0 = incorrecta sin penalización, -0.5 = penalización 50%. NUNCA usar porcentajes enteros ni coma como separador decimal.
            ANTES DE EJECUTAR:
    ...
    @Tool(description = """
            Actualiza una pregunta de tipo opción múltiple existente.
            Requiere: questionId, cmid y los datos actualizados de la pregunta.
            Solo se modifican los campos que se envíen; los demás conservan su valor actual en Moodle.
            Fracciones: decimales sobre 1 con punto como separador. Ejemplos: 1.0 = correcta, 0.0 = incorrecta sin penalización, -0.5 = penalización 50%. NUNCA usar porcentajes enteros ni coma como separador decimal.
            ANTES DE EJECUTAR: muestra al usuario un resumen de los cambios que se van a aplicar y pide confirmación mediante un selector Sí/No. Solo procede si elige Sí.""")
    ...
    @Tool(description = """
            Elimina un slot (pregunta) de un cuestionario. Requiere el cmid del módulo y el slotId. Devuelve la nueva suma de puntos y el nuevo número de preguntas.
            ANTES DE EJECUTAR: muestra al usuario los datos del slot (id, número, nombre de la pregunta si está disponible) y pide confirmación mediante un selector Sí/No. Solo procede si elige Sí.""")
