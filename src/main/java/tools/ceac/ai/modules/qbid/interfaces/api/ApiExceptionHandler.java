...
import tools.ceac.ai.modules.qbid.domain.exception.QbidParseException;
...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleNoResourceFound(NoResourceFoundException ex) {
        System.out.printf("[ApiExceptionHandler] 404 NoResourceFoundException: %s%n", ex.getMessage());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(errorBody(404, ex.getMessage()));
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<Map<String, Object>> handleGeneric(Exception ex) {
        System.out.printf("[ApiExceptionHandler] 500 %s: %s%n",
